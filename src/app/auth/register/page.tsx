...
import { User as UserIcon, Mail, Lock, Facebook, Apple } from "lucide-react";
...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Tampilkan toast berhasil lalu arahkan ke login setelah jeda singkat
        try { toast.success("Registrasi berhasil. Silakan login."); } catch {}
        setTimeout(() => { try { window.location.href = "/auth/login"; } catch {} }, 900);
      }
    } catch {
    ...
    <div className="relative z-10 text-center text-white flex flex-col items-center">
      ...
      <div className="max-w-3xl px-16">
        <h2 className="text-xl font-medium mb-4 leading-snug tracking-wide">
          Bergabung Sekarang.
          <br />
          Kelola Invoice Lebih Mudah.
        </h2>
        <p className="text-white/90 text-base leading-relaxed">
          Dengan <span className="font-semibold">YB Invoice Maker</span>, Anda
          bisa membuat invoice profesional, mencatat pelanggan, produk, dan
  ...
  <div className="flex items-center justify-center p-10 bg-white relative">
    ...
    <SoftCard className="p-12 w-full max-w-xl shadow-lg border border-black/10 rounded-2xl mt-28">
      <h1 className="text-3xl font-semibold mb-1 text-center">Daftar</h1>
      ...
      <form className="space-y-5" onSubmit={onSubmit}>
        <div>
          ...
          <div className="mt-2 relative">
            <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-black/50" />
            <input
              value={name}
